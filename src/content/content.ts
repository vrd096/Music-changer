...
import { createTabCaptureStrategy } from './interception/strategy-tabcapture';
    ...
    try {
      ...
      if (result.success) {
        if (result.sourceNode) {
          pipeline?.connect(result.sourceNode, el);
        }
        pipelineActive = true;
