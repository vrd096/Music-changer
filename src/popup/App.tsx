...
import { TonalityCard } from './components/TonalityCard';
import { SpeedCard } from './components/SpeedCard';
import { EqCard } from './components/EqCard';
import { BpmKeyCard } from './components/BpmKeyCard';
    ...
    <header
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="font-semibold tracking-[0.3px] uppercase border-0 bg-transparent cursor-pointer"
    ...
    <div className="flex-1 px-3 py-2.5" style={{ overflowY: 'auto' }}>
      {pendingHostUrl && (
        <div
          ...
          style={{ color: 'var(--text-secondary)' }}>
          <p style={{ fontSize: '14px' }}>Требуется доступ к сайту</p>
    ...
    <div className="flex flex-col" style={{ height: '100%' }}>
      ...
      {currentPage === 'history' && (
        <div className="flex-1 flex flex-col" style={{ overflowY: 'auto' }}>
          <button
            onClick={() => setCurrentPage('main')}
            className="flex items-center gap-1 mx-3 mt-2 mb-1 border-0 bg-transparent cursor-pointer flex-shrink-0"
            style={{ color: 'var(--accent-secondary)', fontSize: '11px' }}>
            ← Назад
          </button>
          <div className="flex-1" style={{ overflowY: 'auto' }}>
            <HistoryPage isSidePanel={false} />
          </div>
        </div>
      )}
